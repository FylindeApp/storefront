...
import { OptionalAddress, AddressField } from "./types";
...
export const useAddressFormUtils = (countryCode: CountryCode = defaultCountry) => {
	...
	const localizedFields = useMemo(
		() => ({
			countryArea: validationRules?.countryAreaType,
			city: validationRules?.cityType,
			postalCode: validationRules?.postalCodeType,
		}),
		[validationRules],
	);
	...
	const getFieldLabel = useCallback(
		(field: AddressField) => {
			...
			return localizedField && localizedField !== field
				? getLocalizedFieldLabel(field, localizedField)
				: addressFieldMessages[field as AddressFieldLabel];
		},
